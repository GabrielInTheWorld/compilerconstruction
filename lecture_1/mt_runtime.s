#	File: mt_runtime.s
#	adapted for Mini-Tiger by R. Hoeckmann/June 2015
#
...
#	adapted for Simple Tiger by M.Weinhardt@hs-osnabrueck.de
#       --> all function arguments/results are on stack!
#	
#	Concatenate this file `st_runtime.s' with the
#	Tiger-generated MIPS assembly output `*.s'
#	and load the concatenated file into SPIM.
#
#       Example:
#	$ cat st_runtime.s foo.s > foo-run.s
...
# output: $v0 array address

	.text

initArray:
	lw $a0, 0($sp)			# load inputs to registers
	lw $a1, 4($sp)
...
Lrunt2:		
	...
	sw $v0, 0($sp)			# store output to stack
	j $ra
...
# main -- initialize the runtime system, then run the  Simple Tiger program
#	  (which starts at label mt_main)
#
# input:  -
# output: -
...
chr:
	lw $a0, 0($sp)			# load inputs to registers
Lrunt60:					# for direct jump from substring
...
substring:
	...
	addiu $sp,$sp,4
	b Lrunt60				# let `chr' do the work instead
Lrunt42:
